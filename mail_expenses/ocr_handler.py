...
def _find_rpo(text):
    ...
    Встречаемые форматы:
      '1 РПО № 80110619875732'   — с ведущей цифрой и №
      'РПО № 80515419896756, шт.' — без ведущей цифры, с суффиксом ', шт.'
      'РПО: 80110619875732'       — с двоеточием
      'RX123456789RU'             — международный формат
    """
    patterns = [
        # С ведущей цифрой: "1 РПО № 80110619875732"
        r'\d+\s+рпо\s*[№#N]?\.?\s*(\d{14})',
        # Без ведущей цифры: "РПО № 80515419896756, шт."
        # Разделитель между РПО и номером — любые пробелы, №, #, :, точки
        r'рпо\s*[№#N:\.]*\s*(\d{14})',
        # Идентификатор / ШПИ / штрихкод / трек
        r'(?:идентификатор|шпи|штрих[\s\-]?код|трек)[:\s#№N\.]*(\d{14})',
        # Международный формат с ключевым словом
        r'рпо\s*[№#N:\.]*\s*([A-Za-z]{2}\d{9}[A-Za-z]{2})',
        # 14 цифр без ключевого слова (крайний случай)
        r'\b(\d{14})\b',
        # Международный без ключевого слова
...
def _find_amount(text):
    ...
    Встречаемые форматы:
      'ИТОГ =133.60'       — с равно
      'ИТОГ -138.60'       — с минусом (касса показывает расход)
      'ИТОГ    133.60'     — только пробелы (PDF-чек)
      'БЕЗНАЛИЧНЫМИ 133.60'
    Избегаем 'Тариф за пересылку' — это не итоговая сумма.
    """
    # ИТОГ — главный показатель.
    # После слова могут быть пробелы, =, - (минус кассы), затем число.
    # Минус игнорируем — сумма всегда положительная.
    itog = re.search(
        r'итог[оа]?\s{0,30}[=\-]?\s{0,5}[-]?(\d{1,6}[.,]\d{2})',
        text, re.IGNORECASE
    )
    if itog:
        return _parse_amount(itog.group(1))

    # БЕЗНАЛИЧНЫМИ — дублирует итог в кассовом чеке
    beznal = re.search(
        r'безналичными\s{0,30}[=\-]?\s{0,5}[-]?(\d{1,6}[.,]\d{2})',
        text, re.IGNORECASE
    )
    if beznal:
    ...
    # К оплате / Оплачено
    oplata = re.search(
        r'(?:к\s+оплате|оплачено)[:\s=\-]+(\d{1,6}[.,]\d{2})',
        text, re.IGNORECASE
    )
    if oplata:
        return _parse_amount(oplata.group(1))

    # Число с рублями — берём последнее (обычно итог внизу чека)
